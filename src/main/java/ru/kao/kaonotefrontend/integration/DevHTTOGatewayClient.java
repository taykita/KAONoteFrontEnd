...
@Component
@Profile("DEV")
public class DevHTTOGatewayClient implements GatewayClient {
    ...
    public DevHTTOGatewayClient(@Value("${ru.kao.note.front.gateway.url}") String resourceUrl) {
    ...
    public JSONObject executeHttpSend(JSONObject body) throws JSONException {
        ...
        ResponseEntity<String> restDTOResponseEntity = delegate.postForEntity(RESOURCE_URL + "/http-to-server", request, String.class, MediaType.APPLICATION_JSON);
    ...
    @Override
    public JSONObject send(JSONObject body) throws JSONException {
        return executeHttpSend(body);
    }
}
